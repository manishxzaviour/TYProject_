...
float actPos2;
float actAvg;
float constrainedAvg;
float constrainedDemand;
float actAvgBM;
float demand;
...
PID PID1(&Input1, &Output1, &Setpoint1, Pk1, Ik1 , Dk1, DIRECT);    // PID Setup 1 - Balancing
...
//PID PID2(&Input2, &Output2, &Setpoint2, Pk2, Ik2 , Dk2, DIRECT);    // PID Setup 2 - position hold
...
void loop() {
  ...
  if (currentMillis - previousMillis >= 10) {  // start timed event
    ...
    // read encoder postions
    
    Serial1 << "r axis" << 0 << ".encoder.pos_estimate << \n";
    actPos1 = odrive1.readFloat(); 
    Serial1 << "r axis" << 1 << ".encoder.pos_estimate << \n";
    actPos2 = odrive1.readFloat();  

    actAvg = (actPos1 + (actPos2*-1)) / 2;
    ...
    if (mydata_remote.toggleTop == 0) {
        ...
        RFBaccum = (RFBaccum + (RFB/1500));
        actAvg = ((actAvg*5) - (actAvgBM*5));
        ...
        LT = LT / 100;    // convert to turns   
    
        Input1 = pitch;        
        //
        Setpoint1 = pot1+ Output2;
        
        PID1.Compute();
    
        output1 = Output1 + LT;
        output2 = (Output1*-1) + LT;
    
        odrive1.SetVelocity(0, output2);  // differential drive
